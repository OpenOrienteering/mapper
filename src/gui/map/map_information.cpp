...
MapInformation::MapInformation(QWidget* parent, Map* map)
 : QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint)
 , map(map)
{
	setWindowTitle(tr("Map information"));
	
	auto *button_box = new QDialogButtonBox(QDialogButtonBox::Ok, Qt::Horizontal);
	...
	box_layout->addWidget(map_info_tree);
...
void MapInformation::retrieveInformation()
{
	map_information.map_scale = int(map->getScaleDenominator());
	map_information.map_crs = map->getGeoreferencing().getProjectedCRSName();
	auto georeferencing_state = map->getGeoreferencing().getState();
	if (map_information.map_crs == tr("Local") && georeferencing_state == Georeferencing::Geospatial)
		map_information.map_crs = QString::fromLatin1("PROJ.4");
	...
	for (int i = 0; i < map_information.colors_num; ++i)
	{
		...
		if (map_color)
		{
			...
			map_information.colors.push_back(color);
		}
	}
	...
	map_information.fonts_num = int(map_information.font_names.size());
}

void MapInformation::addSymbol(const Symbol* symbol)
{
...
void MapInformation::addTreeItem(const int level, const QString item, const QString value)
{
	auto tree_depth = int(tree_item_hierarchy.size());
	
	Q_ASSERT(level >= 0 && level <= 3);
	Q_ASSERT(level <= tree_depth);
...
inline
const QString MapInformation::getFullSymbolName(const Symbol* symbol) const
{
