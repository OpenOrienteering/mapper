...
{
	pinching = false;
	dragging = false;  // Clear stale drag from touch_pan_only mode
	current_pressed_buttons = 0;
	last_mouse_release_time = QTime::currentTime();
...
#ifdef Q_OS_ANDROID
	if (spen_hover_pauses_rotation && last_spen_hover_time.isValid())
	{
		int hover_elapsed = last_spen_hover_time.msecsTo(QTime::currentTime());
		if (hover_elapsed >= 0 && hover_elapsed < 500)
			return 0;
	}
#endif
...
bool MapWidget::event(QEvent* event)
{
	...
	case QEvent::TabletRelease:
		...
		{
			last_spen_hover_time = QTime::currentTime();
		}
		break;
	...
	case QEvent::TouchUpdate:
	{
		...
		if (multi_touch_active)
			return true;  // Consume trailing single-point events during multi-touch
		break;
	}
	...
	case QEvent::TouchCancel:
		...
		if (multi_touch_active)
		{
			...
			current_pressed_buttons = 0;
			last_mouse_release_time = QTime::currentTime();
			return true;  // Prevent spurious mouse synthesis after gesture
		}
		break;
...
{
#ifdef Q_OS_ANDROID
	// S-Pen hover detection: on Android, MouseMove with no buttons
	// is generated by S-Pen hovering near the screen.
	if (spen_hover_pauses_rotation && event->buttons() == Qt::NoButton)
	{
		last_spen_hover_time = QTime::currentTime();
	}
#endif
