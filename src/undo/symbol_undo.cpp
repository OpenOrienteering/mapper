...
void SymbolUndoStep::saveImpl(QXmlStreamWriter &xml) const
{
	...
	change_element.writeAttribute(literal::type, change_type);
...
void SymbolUndoStep::loadImpl(QXmlStreamReader &xml, SymbolDictionary &)
{
	if (xml.name() == literal::change)
	{
		...
		if (type == static_cast<unsigned>(AddSymbol))
		{
			change_type = AddSymbol;
		}
		else if (type == static_cast<unsigned>(RemoveSymbol))
		{
			change_type = RemoveSymbol;
		}
		else if (type == static_cast<unsigned>(UndefinedChange))
		{
			change_type = UndefinedChange;
		}
		else
		{
			qWarning("Unknown symbol change type %u", type);
		}
	}
}
