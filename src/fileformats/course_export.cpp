...
//const PathObject* CourseExport::findObjectForExport() const
//{
...
const std::vector<ControlPoint> CourseExport::getControlsForExport()
{
    ...
    if (map.getNumSelectedObjects() > 0)
    {
        ...
        qInfo() << "Found control points:" << control_objects.size() << "control codes:" << control_code_objects.size();
        ...
        for (auto* object : control_objects)
        {
            ...
            {
                qInfo() << "Found start:" << object->asPoint()->getCoordF();
                addControl(controls, object, QStringLiteral("S1"));
                continue;
            }
            else if (symbol == finish_symbol)
            {
                qInfo() << "Found finish:" << object->asPoint()->getCoordF();
            ...
            {
                qInfo() << "Found control point:" << object->asPoint()->getCoordF() << "control code:" << control_code_object->asText()->getText();
...
void CourseExport::setProperties(Map& map, const QString& event_name, const QString& course_name, const QString& start_symbol, const QString& finish_symbol)
{
