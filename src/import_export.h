...
{
public:
    ImportExportBase(const QString &path, Map *map, MapView *view) : path(path), map(map), view(view) {}
    virtual ~ImportExportBase() {}
    ...
    QVariant option(const QString &name) const throw (ImportExportException);
 ...
 */
class Importer : public ImportExportBase
{
public:
    Importer(const QString &path, Map *map, MapView *view) : ImportExportBase(path, map, view) {}
    virtual ~Importer() {}

    inline const std::vector<ImportAction> &actions() const { return act; }

    virtual void doImport() throw (ImportExportException) = 0;
    virtual void finishImport() throw (ImportExportException) {}
 ...
 */
class Exporter : public ImportExportBase
{
public:
    Exporter(const QString &path, Map *map, MapView *view) : ImportExportBase(path, map, view) {}
    virtual ~Exporter() {}

    virtual void doExport() throw (ImportExportException) = 0;
};
